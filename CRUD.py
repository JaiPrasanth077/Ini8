...
def update_registration(registration_id):
    cursor = connection.cursor()
    try:
        column_name = input("Enter the column name to update (Name, Email, dob, contactnum): ")
        if column_name.lower() not in ['name', 'email', 'dob', 'contactnum']:
            print("Invalid column name.")
            return
        new_value = input(f"Enter new value for {column_name}: ")
        cursor.execute(f"UPDATE Registration SET {column_name} = %s WHERE ID = %s", (new_value, registration_id))
        connection.commit()
